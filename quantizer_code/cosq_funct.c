...
float expected_distortion(float centroid_array[], int array_length, unsigned int index, float point, float epsilon, int bits){
    double distortion = 0;
    float trans_prob = 0;
...
float * iteration(float* centroids, int centroid_len, float* training_points, int training_point_len, int count, float epsilon, int bits, int print_distortion){
    ...
    float distortion = 0;
    int i = 0;
    float stopping_criteria = 0.00005 * bits;
    do
    {
    ...
    } while((fabs((distortion - curr_distortion)/distortion) > stopping_criteria) && (++i < 125));
