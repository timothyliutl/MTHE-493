...
centroids = [3,40,100, 210, 123]
cent_len = len(centroids)
centroids = (c_float * len(centroids))(*centroids)

arr = [np.random.randint(0,255) for i in range(200)]
...
buffer = my_functions.calc_partitions(arr, arr_len, centroids, cent_len, 0.005, 8, return_arr)
...
return_iter = my_functions.calc_centroids(arr1, len(arr1), centroids, cent_len, arr, len(arr), 0.005, 8, return_arr2)
